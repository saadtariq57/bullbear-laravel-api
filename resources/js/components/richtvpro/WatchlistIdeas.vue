<template>
    <div class="container my-4">
        <div class="text-center">
            <h1 class="fw-bold fs-1 text-uppercase">Watchlists Ideas</h1>
            <div class="border-heading d-inline-block mt-3 mb-4"></div>
        </div>
        <div class="row" v-if="watchlists">
            <!-- {{ watchlists }} -->
            <div class="col-lg-6 col-md-12 my-4" v-for="watchlist in watchlists" v-show="watchlist.featured == 1">
                <div
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <h3 class="fs-18 cursor-pointer" @click="showWatchlistModal(watchlist)"><b>{{ watchlist.title
                                }}</b></h3>
                        ...
                        <button class="btn btn-primary btn-sm pe-3 h-75" @click="clonewatchlist(watchlist)" v-else>
                    ...
                    <div class="table-responsive watchlist-dash-table">
                        <table class="table stock-market-table1">
                            <thead>
                                <tr>
                                    <th scope="col" class="sticky-side position-sticky text-black ps-0">
                                        Name</th>
            ...
            symbolData.symbol.company_name }}</span>
                                    ...
                                    <td class="gray lh-sm text-end" id="symbol-price">
                                        <div v-if="!symbolData.symbol.stats">
                                            <span style="margin-bottom:4px;display:block;width: 50px;text-align:right;">
                                                <Skeletor height="15" />
                                            </span>
                                        ...
                                        <div v-else>
                                            ...
                                            <div :class="textChangeClasses(symbolData)">
                                                <span>{{ formatNumber(symbolData.symbol.stats.regularMarketChange)
                                                    }}</span>
                                                <span>{{
        ...
        <div class="d-flex gap-3" v-else>
            <Skeletor style="width: 100%; height: 70vh;" class="rounded-0" />
            <Skeletor style="width: 100%; height: 70vh;" class="rounded-0" />
        </div>
        <div class="modal fade" ref="viewWatchlistModal" id="viewWatchlisteModal" tabindex="-1"
                ...
                <div class="modal-content" v-if="watchlistModal">
                    ...
                    <div class="modal-body">
                        <div class="mt-4 table-responsive">
                            <DataTable class="display table watchlist-new-table fw-bold" id="newsTable"
                                ...
                                <tbody>
                                    <tr :key="symbolData.id" v-for="symbolData in watchlistModal.watchlist_symbols"
                                        class="position-relative"
                                        :class="symbolData.symbol.news.title != null ? 'table-news-row' : ''">
                                        <td
                                            :class="symbolData.symbol.stats ? 'table-news-watchlist-row' : 'align-middle'">
                                            {{ symbolData.symbol.name }} <p class="small-para fw-bold mb-0">{{
            ...
            symbolData.symbol.stats.regularMarketDayRange }}</span>
                                        ...
                                        <div class="fw-bold fs-16 py-0" v-if="symbolData.symbol.news.title"
                                            :class="symbolData.symbol.news.title != null ? 'watchlist-table-news' : ''">
                                            <a :href="symbolData.symbol.news.link" target="_blank"
                                                class="text-black text-oneline watchlist-new">
