<template>
  <div ref="reactionModal" class="modal fade" id="reactionPostModal" tabindex="-1" aria-labelledby="reactionPostModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header pb-0 border-0">
          <h1 class="modal-title fs-5" id="reactionPostModalLabel">Reactions</h1>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" @click="$emit('close-modal')"></button>
        </div>
        <div class="mt-2 reactions-post-wrapper mx-3">
          <ul class="nav border-0 nav-tabs gap-1 flex-nowrap" id="reactions-post-Followers-tab" role="tablist">
            <li class="nav-item" role="presentation">
              <button class="nav-link border-0 fs-6 text-black px-2 py-2 d-flex btn-feed-hover reactions-post-nav-btn text-nowrap active"
                      id="allreactions-tab" data-bs-toggle="tab" data-bs-target="#allreactions-tab-pane" type="button" role="tab"
                      aria-controls="allreactions-tab-pane" aria-selected="false">
            ...
            <li class="nav-item" role="presentation" v-for="(reaction, type) in activeItem.reactionData" :key="type">
              <button class="nav-link border-0 fs-6 text-black px-2 py-2 d-flex btn-feed-hover reactions-post-nav-btn text-nowrap"
        ...
        <div class="modal-body">
          <div class="list-group">
            <div class="tab-content" id="myTabContent">
              <div class="tab-pane fade show active" id="allreactions-tab-pane" role="tabpanel" aria-labelledby="allreactions-tab" tabindex="0">
                <template v-for="(reaction, type) in activeItem.reactionData">
                  <a href="#" class="list-group-item list-group-item-action px-0 py-0 d-flex align-items-center gap-3 border-0"
                     v-for="detail in reaction.details" :key="`${type}-${detail.userId}`">
                    <div class="position-relative">
                      <img :src="detail.userImage" alt="" width="40" height="40">
              ...
              <div class="tab-pane fade" v-for="(reaction, type) in activeItem.reactionData" :id="`${type}reactions-tab-pane`" role="tabpanel" :aria-labelledby="`${type}reactions-tab`" tabindex="0" :key="type">
                <a href="#" class="list-group-item list-group-item-action px-0 py-0 d-flex align-items-center gap-3 border-0"
                   v-for="detail in reaction.details" :key="`${type}-${detail.userId}`">
                  <div class="position-relative">
                    <img :src="detail.userImage" alt="" width="40" height="40">
