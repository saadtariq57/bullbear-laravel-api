<template>
  <div class="container my-4">
    <div class="row">
      <div class="col-lg-8">
        <div class="mb-5">
          <h1 class="fs-1 fw-bold icon-heading">Upcoming Earnings Releases</h1>
        </div>
        <div class="earning">
                          ...
                          <template v-if="allEarnings[tabName] && allEarnings[tabName].length > 0">
                            <template v-for="(item, index) in allEarnings[tabName].slice(0, visibleRows[tabName])" :key="item.id">
                              ...
                              <tr>
                                  ...
                                  <div class="d-flex gap-1 align-items-center">
                                    ...
                                    <div class="w-100">
                                      <a :href="`/quotes/${item.symbol}`" class="text-decoration-none">
                                        <span class="text-primary fw-bold">{{ item.symbol }}</span>
                                ...
                                <td class="text-end" :class="getColorClass(item.revenue, item.revenue_estimated)">{{ formatLargeNumber(item.revenue) }}</td>
                                <td class="text-start fw-5 ps-1">/ {{ formatLargeNumber(item.revenue_estimated) }}</td>
...
export default {
  ...
  methods: {
    ...
    getColorClass(actual, estimate) {
      ...
      if (actual > estimate) return 'text-success';
      if (actual < estimate) return 'text-danger';
      return '';
    },
