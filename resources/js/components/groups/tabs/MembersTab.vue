<template>
    <div id="members-tab-pane">
      <div class="px-3 pt-2">
          ...
          <div class="d-flex gap-3">
            <div v-if="followersData">
              <div v-if="userData.id !== member.id">
                ...
                <button v-else @click="handleFollowUser(member.id, followersCount)" type="button" class="btn btn-primary fs-6 fw-5 px-3">
                  Follow
                </button>
              </div>
            </div>
        ...
        <!-- Modal -->
        <div class="modal fade" ref="memberEditModal" id="memberEdit" tabindex="-1" aria-labelledby="memberEditLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Edit Member</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" @click="resetEditMember"></button>
              </div>
              <div class="modal-body">
                <div v-if="editMember">
                  <form @submit.prevent="handleSubmit">
                    <div class="mb-3">
                      <input type="hidden" class="form-control border shadow-sm" id="editMemberGroupId" v-model="group_id">
                    </div>
                    <div class="mb-3">
                      <label for="editMemberId" class="form-label">ID</label>
                      <input type="text" class="form-control border shadow-sm" id="editMemberId" disabled v-model="editMember.id">
                    </div>
                    <div class="mb-3">
                      <label for="editMemberName" class="form-label">Name</label>
                      <input type="text" class="form-control border shadow-sm text-capitalize" id="editMemberName" disabled v-model="editMember.name">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-select border shadow-sm" id="editMemberStatus" v-model="editMember.pivot.status">
