<template>
  <div class="pricing-wrapper bg-white px-2 px-md-4 py-3 mt-4 border-1 border rounded-2 shadow-sm">
        ...
        <div class="alert alert-light mb-5 mb-xl-9 shadow-sm" role="alert">
          <div class="d-flex justify-content-center align-items-center">
            ...
            <div class="form-check form-switch rtv-form-switch rtv-form-switch-md">
              <input class="form-check-input" type="checkbox" role="switch" id="rtv-plans" v-model="showYearly" @change="toggleBillingCycle">
    ...
    <div class="row mt-5 pt-5 mb-3 gy-4 justify-content-center">
      <div v-for="plan in subscriptionPlans" :key="plan.id" class="col-xl-4 col-lg-6 col-md-12 px-1">
        <div class="card shadow rounded-4 h-100">
          ...
          <ul class="list-group list-group-flush membership-checks-list px-1 my-5">
            <li v-for="feature in plan.features" :key="feature.id" class="list-group-item py-2">
                {{ feature.feature_name }} {{ feature.feature_type }} {{ feature.limit }}  
            </li>
          </ul>
          <div v-if="plan.currentSubscription">
          </div>
          <button v-if="plan.currentSubscription && plan.currentSubscription.stripe_price == plan.stripe_monthly_price_id && showYearly == false" class="btn w-100 btn-basic-plan py-4 fs-4 fw-5 align-self-end" readonly>Current Plan</button>
          <button v-else-if="plan.currentSubscription && plan.currentSubscription.stripe_price == plan.stripe_yearly_price_id && showYearly == true" class="btn w-100 btn-basic-plan py-4 fs-4 fw-5 align-self-end" readonly>Current Plan</button>
          <button v-else @click="redirectToCheckout(plan)" class="btn w-100 btn-basic-plan py-4 fs-4 fw-5 align-self-end">Subscribe</button>
        </div>
      </div>
    </div>
  </div>
